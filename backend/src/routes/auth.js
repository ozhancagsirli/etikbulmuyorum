...
router.post('/refresh', async (req, res, next) => {
  try {
    ...
    const { rows } = await pool.query(
      `SELECT s.id, u.id AS user_id, u.role, u.is_banned
...
router.post('/register', authLimiter, async (req, res, next) => {
  try {
    ...
    if (!name || !email || !password) return res.status(400).json({ error: 'Ad, email ve şifre gerekli.' });
    if (password.length < 6) return res.status(400).json({ error: 'Şifre en az 6 karakter olmalı.' });

    const existing = await pool.query('SELECT id FROM users WHERE email = $1', [email]);
    if (existing.rows.length) return res.status(409).json({ error: 'Bu email zaten kayıtlı.' });
...
router.post('/login', authLimiter, async (req, res, next) => {
  try {
    ...
    if (!rows.length || !rows[0].password_hash) return res.status(401).json({ error: 'Email veya şifre hatalı.' });
    if (rows[0].is_banned) return res.status(403).json({ error: 'Hesabınız askıya alınmıştır.' });
