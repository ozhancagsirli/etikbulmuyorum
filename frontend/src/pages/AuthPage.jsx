    ...
    <div style={{ maxWidth: 400, margin: '48px auto', padding: '0 16px' }}>
      ...
      <div style={{ textAlign: 'center', marginBottom: 28 }}>
        <Link to="/" style={{ display: 'inline-flex', alignItems: 'center', gap: 8, fontWeight: 800, fontSize: 20, letterSpacing: -0.4 }}>
          ...
          <span style={{ color: '#46A53E' }}>etik</span><span style={{ color: '#111827' }}>bulmuyorum</span>
        </Link>
      ...
      <div style={{ background: 'white', borderRadius: 16, border: '1px solid #e5e7eb', boxShadow: '0 4px 24px rgba(0,0,0,0.06)', overflow: 'hidden' }}>
        ...
        <form onSubmit={handleSubmit} style={{ padding: '20px 24px 24px', display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          {tab === 'register' && (
            <div>
              ...
              <input value={form.name} onChange={set('name')} placeholder="Adınızı girin" required
                style={inp()}
                onFocus={e => { e.target.style.borderColor='#46A53E'; e.target.style.background='white'; }}
          ...
          <button type="submit" disabled={loading} style={{
            ...
            onMouseLeave={e => { if (!loading) e.currentTarget.style.background='#46A53E'; }}
          >
          ...
          {tab === 'login' && (
            <p style={{ textAlign: 'center', fontSize: 13, color: '#6b7280' }}>
              Hesabınız yok mu?{' '}
              <button type="button" onClick={() => setTab('register')} style={{ background: 'none', border: 'none', color: '#46A53E', fontWeight: 600, cursor: 'pointer', fontSize: 13 }}>
                Kayıt Ol
              </button>
            </p>
          )}
