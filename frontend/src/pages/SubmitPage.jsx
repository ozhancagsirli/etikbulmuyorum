...
export default function SubmitPage() {
  ...
  if (!user) return (
    <div style={{ background: 'white', borderRadius: 12, padding: 48, textAlign: 'center', border: '1px solid #e0e0e0', maxWidth: 480, margin: '0 auto' }}>
      ...
      <Link to="/" style={{ color: '#dc2626', fontWeight: 600, fontSize: 15 }}>Ana sayfaya dön</Link>
    </div>
  );
        ...
        {[1, 2, 3].map(n => (
          <div key={n} style={{ display: 'flex', alignItems: 'center', gap: 8, flex: n < 3 ? 1 : 'none' }}>
            <div style={{
              ...
              background: step >= n ? '#dc2626' : '#f0f0f0',
            ...
            {n < 3 && <div style={{ flex: 1, height: 2, background: step > n ? '#dc2626' : '#f0f0f0', borderRadius: 1 }} />}
          </div>
        ))}
      </div>
        ...
        {step === 1 && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 8 }}>
                {categories.map(c => (
                  <button key={c.id} type="button" onClick={() => setForm(f => ({ ...f, categoryId: c.id }))} style={{
                    ...
                    borderColor: String(form.categoryId) === String(c.id) ? '#dc2626' : '#e0e0e0',
                    background: String(form.categoryId) === String(c.id) ? '#fef2f2' : 'white',
                    color: String(form.categoryId) === String(c.id) ? '#dc2626' : '#444',
            ...
            <div ref={subjectRef} style={{ position: 'relative' }}>
              <label style={{ display: 'block', fontWeight: 600, fontSize: 13, marginBottom: 8 }}>
                👤 Kişi / Firma Adı <span style={{ color: '#dc2626' }}>*</span>
              </label>
              <input
                value={form.subject}
            ...
            <button type="button" onClick={nextStep} style={{
              ...
              background: '#dc2626', color: 'white', fontWeight: 700, fontSize: 15, cursor: 'pointer',
            }}>Devam Et →</button>
          </div>
        )}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            {selectedCategory && (
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 14px', background: '#fef2f2', borderRadius: 8, border: '1px solid #fecaca' }}>
                ...
                <span style={{ fontSize: 14, color: '#dc2626', fontWeight: 600 }}>{selectedCategory.name_tr}</span>
            ...
            <div>
              <label style={{ display: 'block', fontWeight: 600, fontSize: 13, marginBottom: 8 }}>
                Ne Yaşandı? <span style={{ color: '#dc2626' }}>*</span>
              </label>
        ...
        {step === 3 && (
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            ...
            <label style={{ display: 'flex', gap: 12, alignItems: 'center', cursor: 'pointer', padding: '14px 16px', background: '#f8f9fa', borderRadius: 8, border: '1.5px solid', borderColor: form.isAnonymous ? '#dc2626' : '#eee' }}>
              <input type="checkbox" checked={form.isAnonymous} onChange={set('isAnonymous')} style={{ width: 18, height: 18, accentColor: '#dc2626' }} />
              <div>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              ...
              <button type="submit" disabled={submitting} style={{
                ...
                background: submitting ? '#ccc' : '#dc2626',
