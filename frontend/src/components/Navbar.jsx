      ...
      <nav style={{ background: 'white', borderBottom: '1px solid #e5e7eb', position: 'sticky', top: 0, zIndex: 100, boxShadow: '0 1px 3px rgba(0,0,0,0.06)' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto', padding: '0 12px', height: 52, display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginLeft: 'auto', flexShrink: 0 }}>
            <button onClick={() => setMobileSearch(s => !s)} id="mobile-search-btn" style={{ background: 'none', border: 'none', fontSize: 18, cursor: 'pointer', color: '#6b7280', padding: '4px', display: 'none' }}>🔍</button>

            {user ? (
              <>
                <Link to="/bildir" style={{ background: '#FF4500', color: 'white', padding: '6px 12px', borderRadius: 20, fontWeight: 700, fontSize: 12, flexShrink: 0 }}>
                  + Bildir
                </Link>
                <div style={{ position: 'relative' }}>
                  ...
                  {menuOpen && (
                    <div style={{ position: 'absolute', right: 0, top: 42, background: 'white', borderRadius: 12, border: '1px solid #e5e7eb', boxShadow: '0 8px 24px rgba(0,0,0,0.12)', minWidth: 180, overflow: 'hidden', zIndex: 200 }}
                      ...
                      <button onClick={() => { logout(); navigate('/'); }} style={{ display: 'block', width: '100%', padding: '10px 16px', textAlign: 'left', background: 'none', border: 'none', color: '#ef4444', fontSize: 13, cursor: 'pointer' }}>🚪 Çıkış Yap</button>
                    </div>
                  )}
                </div>
              </>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                <Link to="/giris" style={{ color: '#374151', fontSize: 12, fontWeight: 600, padding: '6px 12px', borderRadius: 20, border: '1.5px solid #e5e7eb', whiteSpace: 'nowrap' }}>
                  Giriş
                </Link>
                <Link to="/giris?tab=register" style={{ background: '#FF4500', color: 'white', fontSize: 12, fontWeight: 700, padding: '6px 12px', borderRadius: 20, whiteSpace: 'nowrap' }}>
                  Kayıt
                </Link>
              </div>
            )}
